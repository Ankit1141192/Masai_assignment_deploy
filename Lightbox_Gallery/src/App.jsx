  ...
  return (
    <div className="p-6">
      <div className="grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-4">
        {images.map((img) => (
          <img
            ...
            src={img.src}
            alt={img.alt}
            className="w-full h-40 object-cover cursor-pointer rounded-lg transition-transform duration-300 hover:scale-105"
      ...
      {selectedImage && (
        ...
        >
          <div
            className="flex justify-center items-center max-w-[90%] max-h-[90%]"
            onClick={(e) => e.stopPropagation()}
          >
            <img
              src={selectedImage.src}
              alt={selectedImage.alt}
              className="max-w-[70vw] max-h-[70vh] rounded-xl cursor-pointer shadow-[0_0_30px_10px_rgba(0,123,255,0.8)] transition-transform duration-300 hover:scale-105"
              onClick={closeModal}
            />
          </div>
        </div>
